// GM (Geman-McClure) baseline — IPC paper comparator [7], "provided by the gtsam library".
// The IPC author compared against GM but did NOT release a GM tester. This file is a VERBATIM
// clone of dcs_3D.cpp with the SINGLE algorithmic change being the robust m-estimator:
//   noiseModel::mEstimator::DCS::Create(th)  ->  noiseModel::mEstimator::GemanMcClure::Create(th)
// GM's shape parameter is sourced the same way DCS sources its threshold (th = Chi2inv(alpha,dof)).
// NOTE (S3 tuning): the author's GM "best fixed parameter" was not released — confirm/tune this for G1.
#include "utils.hpp"
...
int main(int argc, char **argv)
{
  ...
  for (auto& factor : *graph)
  {
    ...
    if ( std::abs(delta) > 1 )
    {
        ...
        auto robust_noise = noiseModel::Robust::Create(noiseModel::mEstimator::GemanMcClure::Create(th), noise);
